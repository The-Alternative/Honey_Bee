...
class Time_listState extends State<Time_list> {
  ...
  DatabaseHelper _adbHelper = DatabaseHelper();
  ...
  @override
  void initState() {
    _adbHelper.initializeDatabase().then((value) {
                        ...
                        FloatingActionButton.extended(
                          onPressed: () {
                            ...
                            if ('${selectedDate.toLocal()}'.split(' ')[0] ==
                              ...
                              _adbHelper.insertRecordMedicine(medicine);
  ...
  addCardListView() {
    Future<List<MedicineInfo>> cardListFuture = _adbHelper.getAllIds('');
  ...
  addDaysListView() {
    Future<List<MedicineDays>> daysFuture = _adbHelper.getmidDayesList();
  ...
  getselectCardListView(String name) {
    Future<List<MedicineInfo>> cardListFuture = _adbHelper.getAllIds(name);
    cardListFuture.then((cardList) {
      setState(() {
        //debugPrint("list has change");
        this.cardList = cardList;
  ...
  void _update(BuildContext context, MedicineInfo card_info, DateTime selectedDate,
    ...
    print(forma);
    int dayId =
    await _adbHelper.insertDayes(MedicineDays(forma, int.parse(forma2)));
    _adbHelper.updateDayTimes(MedicineTimes(
  ...
  void _delete(BuildContext context, MedicineInfo card_info) async {
    int result = await _adbHelper.deleteDayTimes(card_info.timesId, card_info.dayesId);
    if (result != 0) {
      // _alarmHelper.d
  ...
  getListViewPerson() {
    _PatientList = _adbHelper.getPatientList();
